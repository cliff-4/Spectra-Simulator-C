...
scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=0.0001, steps_per_epoch=20, epochs=1)#torch.optim.lr_scheduler.StepLR(optimizer, step_size=6, gamma=0.3)
...
def categorize(par,num_cl):
    ...
    print(f"categorizing {par=} into {num_cl=} classes")
    par = par*num_cl
    par = np.floor(par)
    par = par.astype('int')
    par = np.where(par<num_cl,par,num_cl-1)
    #par = to_categorical(par,num_classes=num_cl)
    print(f"par shape after categorizing: {par.shape}, par min: {par.min()}, par max: {par.max()}")
    return par
...
#####################################################################################################################    
        ...
        # print(X.shape)
        # print(end_time)
        print(f'Time taken for loading data= {end_time-start_time:.2f} s')
...
#             X.append(data[i,:35692])
        ...
        print(f"Dnu before: {Dnu[:10]}")
        Dnu = np.clip(Dnu, 0, num_classes_dnu-1)
        print(f"Dnu after: {Dnu[:10]}")
        print(f"Dp before: {Dp[:10]}")
        Dp = np.clip(Dp, 0, num_classes_dp-1)
        print(f"Dp after: {Dp[:10]}")
        print(f"q before: {q[:10]}")
        q = np.clip(q, 0, num_classes_q-1)
        print(f"q after: {q[:10]}")
        print(f"acr before: {acr[:10]}")
        acr = np.clip(acr, 0, num_classes_acr-1)
        print(f"acr after: {acr[:10]}")
        print(f"aer before: {aer[:10]}")
        aer = np.clip(aer, 0, num_classes_aer-1)
        print(f"aer after: {aer[:10]}")
        print(f"a3 before: {a3[:10]}")
        a3 = np.clip(a3, 0, num_classes_a3-1)
        print(f"a3 after: {a3[:10]}")
        print(f"inc before: {inc[:10]}")
        inc = np.clip(inc, 0, num_classes_inc-1)
        print(f"inc after: {inc[:10]}")
